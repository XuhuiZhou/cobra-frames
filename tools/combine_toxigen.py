...
def main():
    ...
    df, paths = combine_toxigen(inference_toxigen_path, range(range_start, range_end))
    ...
    print("Number of rows with statementCheck == 'yes': {}".format(len(df_yes)))
    # output the number of rows with statementCheck == 'yes' and no missing values
    print("Number of rows with statementCheck == 'yes' and no missing values: {}".format(len(df_complete)))
