...
def main():
    ## Hyperparameters
    file = "data/inference_data/toxigen_explanations_v2/toxigen_explanations_val.csv"
    df = pd.read_csv(file)
    ...
    # )
    saved_mturk_file = (
        "data/inference_data/toxigen_explanations_v2/toxigen_mturk_val.csv"
    )
    ...
    if sample_num:
        sampled_statements = np.random.choice(
            df["statement"].unique(), sample_num
        )
