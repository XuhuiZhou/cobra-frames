...
def obtain_reference(df, s_var):
    s_var_unique = list(
        set([i.split(".")[1][:-11] for i in s_var])
    )  # From Answer.xxxSuggestion2 to xxx
    reference_dict = {}
    for i in s_var_unique:
        ...
        for index, v in enumerate(values):
            if index % 2 == 1:
                reference_dict[
                    "Answer." + i + "Suggestion" + str(index // 2 + 1)
...
scores, workers = calculate_scores_accelerate(
    worker_suggestions, suggestions_reference, suggestions_var
)
