...
"""
This version includes toxicgen
"""
...
def addExamplesToPost(p, examples, n=10):
    formattedExamples = "\n\n".join(
        [
            "\n".join(
                [formatting[v].format(e[v]) for v in variables if e[v] and not pd.isnull(e[v])]
...
def formatPosts(p, preamble="", **kwargs):
    """
    Combine instructions (preamble), demonstrations (examples), and statement to obtain the
    final prompt. 
    """
    # out = preamble+"\n\nStatement: "+p+"\nSocial context:"
    ...
    # S
    f = [
        formatting[v].format(p[kwargs[v + "_col"]]) for v in variables
...
def obtain_lis_spk_id(df, groups):
    pass
...
def main(args):
    ...
    posts["examples"] = posts[args.statement_col].apply(
        addExamplesToPost, examples=examples, n=args.n_examples
    )
    groups = ['asian', 'mexican', 'chinese', 'lgbtq', 'physical_dis', 'muslim', 'mental_dis', 
    'latino', 'black', 'middle_east', 'native_american', 'women', 'jewish']
    posts = obtain_lis_spk_id(posts, groups)
