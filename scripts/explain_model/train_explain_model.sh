...
if [[ $EXP == "train_xxl" ]]; then
    ...
    python sbf_modeling/train.py --gin_file="scripts/explain_model/explain_model.gin" --gin_file="exp/configs/train_explain_model_base_2_epoch.gin" --gin.MODEL_DIR="'.log/explain-model-base'" --gin.MODE="'deployment'"
elif [[ $EXP == "train_small" ]]; then
    python sbf_modeling/train.py --gin_file="scripts/explain_model/explain_model.gin" --gin_file="exp/configs/train_explain_model_small_2_epoch.gin" --gin.MODEL_DIR="'.log/explain-model-small'" --gin.MODE="'deployment'"
else
