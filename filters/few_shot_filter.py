...
class T5_filter:
    ...
    def __init__(self, args, examples) -> None:
        ...
        self.revFormat = {
            v.replace(" {}[/]", ""): k for k, v in self.formatting.items()
        }
    ...
    def formatPosts(self, p, preamble="", **kwargs):
        ...
        f = [
            self.formatting[v].format(p[kwargs[v + "_col"]])
            for v in self.variables
            if v + "_col" in kwargs
            and kwargs[v + "_col"] in p
            and p[kwargs[v + "_col"]] != ""
        ]
        out = preamble + p["examples"] + "\n\n" + "\n".join(f) + "\n"
        out += (
            self.formatting[
                self.variables[self.variables.index(self.end_variable) + 1]
            ]
            .split("{}")[0]
            .strip()
        )
    ...
    def generate_table(self, posts):
        ...
        posts["examples"] = posts["statement"].apply(
            self.addExamplesToPost,
            examples=self.examples,
            n=self.args.n_examples,
        )
        fPosts = posts.apply(
            self.formatPosts,
            **args.__dict__,
...
def main(args):
    ...
    scores = t5_gen.calculate_scores(
        posts["speakerIdenRating_model"], posts["Answer.speakerIdenRating"]
    )
    print(scores)
...
if __name__ == "__main__":
    ...
    p.add_argument(
        "--input_file", default="./data/cache/annotation_summary.csv"
    )
    p.add_argument(
        "--example_file",
        default="./data/prompts/examples.t5.contextCritic_Anno.csv",
        type=str,
    )
    ...
    p.add_argument(
        "--output_file", default="./data/cache/annotation_summary.t5.csv"
    )
    args = p.parse_args()
    main(args)
