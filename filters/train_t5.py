...
def get_dataset(tokenizer, data_dir, type_path):
    return CustomDataset(
        tokenizer=tokenizer, data_dir=data_dir, type_path=type_path
    )
...
def evaluate(loader, model):
    ...
    for batch in tqdm(loader):
        ...
        dec = [
            int(model.tokenizer.decode(ids).replace("<pad> ", ""))
            for ids in outs
        ]
        target = [
            int(model.tokenizer.decode(ids).split("</s>")[0])
...
if __name__ == "__main__":
    ...
    valid_dataset = CustomDataset(
        model.tokenizer, data_dir=args.data_dir, type_path="valid"
    )
    ...
    test_dataset = CustomDataset(
        model.tokenizer, data_dir=args.data_dir, type_path="test"
    )
